...
test.describe("Auto-approve all — Shift+Tab toggle", () => {
	...
	test.skip("Shift+Tab toggles auto-approve-all setting", async ({ terminal }) => {
		await waitForChatReady(terminal)
...
// ---------------------------------------------------------------------------
// Act task: file edit permission prompt  @live
// ---------------------------------------------------------------------------
test.describe("Act task — file edit permission prompt @live", () => {
	test.use({
		program: { file: CLINE_BIN, args: [] },
		...TERMINAL_WIDE,
		env: clineEnv("claude-sonnet-4.6"),
	...
	test.skip("shows Save/Reject buttons when editing files", async ({ terminal }) => {
		await waitForChatReady(terminal)
		await submitPrompt(terminal, "append yo to the end of the README.md")
		await expectVisible(terminal, /save|reject/i, { timeout: 60_000 })
		await approvePermission(terminal)
		await waitForTaskCompleted(terminal)
		await waitForTaskButtons(terminal)
		await exitAfterTask(terminal)
	})
})
...
test.describe("Subagents @live", () => {
	...
	test.skip("shows subagent indicator during subagent tasks", async ({ terminal }) => {
		await waitForChatReady(terminal)
		await submitPrompt(terminal, "summarize 3 root files in your working directory. use subagents")
		await expectVisible(terminal, "Cline is running subagents", {
			timeout: 60_000,
		})
		await waitForTaskCompleted(terminal, 120_000)
	})
})
...
test.describe("Web tools @live", () => {
	...
	test.skip("asks permission to use web fetch tool", async ({ terminal }) => {
		await waitForChatReady(terminal)
		await submitPrompt(terminal, "summarize this web page in one sentence: https://cline.bot/")
		await expectVisible(terminal, "Cline fetched content from this URL:", {
			timeout: 30_000,
		})
	})
})
